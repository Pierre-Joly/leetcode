...
func twoSum(nums []int, target int) []int {
	seen := make(map[int]int) // map to store the numbers and their indices

	for i, num := range nums {
		diff := target - num // calculate the difference needed to reach the target
		if index, ok := seen[diff]; ok {
			// If the difference is found in the map, return indices of the pair
			return []int{index, i}
		}
	...
	return nil // return nil if no two numbers sum up to the target
}
