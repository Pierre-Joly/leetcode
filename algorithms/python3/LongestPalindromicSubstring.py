...
class Solution:
    def longestPalindrome(self, s: str) -> str:
        ...
        palindrome_radius = [0] * length
        center = right_boundary = 0

        for current_position in range(1, length - 1):
            mirror = 2 * center - current_position

            if current_position < right_boundary:
                palindrome_radius[current_position] = min(right_boundary - current_position, palindrome_radius[mirror])
            
            while transformed[current_position + palindrome_radius[current_position] + 1] == transformed[current_position - palindrome_radius[current_position] - 1]:
                palindrome_radius[current_position] += 1

            if current_position + palindrome_radius[current_position] > right_boundary:
                center = current_position
                right_boundary = current_position + palindrome_radius[current_position]

        max_length, center_index = max((n, i) for i, n in enumerate(palindrome_radius))
